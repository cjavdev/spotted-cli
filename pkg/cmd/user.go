...
func handleUsersRetrieveProfile(ctx context.Context, cmd *cli.Command) error {
	...
	_, err = client.Users.GetProfile(ctx, cmd.Value("user-id").(string), options...)
	if err != nil {
		return err
	}
	...
	return ShowJSON(os.Stdout, os.Stderr, "users retrieve-profile", obj, format, explicitFormat, transform)
}
